...
\subsection{Medical Diagnosis (MIMIC-III)}

Table~\ref{tab:mimic} reports the complete comparison on the MIMIC-III in-hospital mortality prediction task. The closed-loop consistently outperforms all baselines in average cost.

\begin{table}[ht]
\centering
\caption{MIMIC-III in-hospital mortality prediction (129 admissions)}
\label{tab:mimic}
\begin{tabular}{lccc}
\toprule
Method & Avg Cost (\$) & Improvement (\%) & ECE \\
\midrule
\textbf{Closed-loop} & \textbf{850.85} & --- & 0.082 \\
Threshold Moving & 1,041.34 & 18.3 & 0.085 \\
Bayesian Decision & 1,041.34 & 18.3 & 0.085 \\
Raw Threshold & 1,041.34 & 18.3 & 0.085 \\
Cost-Sensitive Platt & 1,073.82 & 20.8 & 0.076 \\
Static Platt & 1,073.82 & 20.8 & 0.076 \\
Temperature Scaling & 1,073.82 & 20.8 & 0.076 \\
Online Platt & 1,119.57 & 24.0 & 0.068 \\
Adaptive Calibration & 1,119.57 & 24.0 & 0.068 \\
Static Isotonic & 1,198.45 & 29.0 & 0.071 \\
\bottomrule
\end{tabular}
\vspace{4mm}
\par\noindent\textit{Note:} The closed-loop achieves an 18.3\% cost reduction over the best baseline (Threshold Moving). The 95\% bootstrap confidence interval for the cost reduction is [5.2\%, 31.4\%], confirming statistical significance (see main text Section~5.2.4).
\end{table}
...
\begin{itemize}
    ...
    \item Pre-generated synthetic datasets (HDF5 format, DOI: to be assigned upon publication)
